...
const rightTurnJson = {
  "type": "right_turn_block",        // ← Required!
...
Blockly.Blocks['right_turn_block'] = {
  init: function() {
    this.jsonInit(rightTurnJson);
  }
};
...
Blockly.Blocks['left_turn_block'] = {
  init: function() {
    this.jsonInit(leftTurnJson);
  }
};
...
Blockly.Blocks['printf_block'] = {
  init: function() {
    this.jsonInit(printfJson);
  }
};


Blockly.C.forBlock['printf_block'] = function(block, generator) {
  const condition = Blockly.C.valueToCode(block, 'DELTA',  Blockly.C.ORDER_NONE) || 0;
  // Atomic order is appropriate for a literal
  return 'printf(' + condition.replace(/(\r\n|\n|\r)/gm,"") + ');';
};
